...
public Data_Ruangan() {
    ...
    // Inisialisasi peta jenis tari dan ID tari
    // Inisialisasi peta jenis tari dan instruktur tari
...
private boolean isRoomNameUnique(String namaRuangan) {
    try {
        ...
        if (conn != null) {
            ...
            try (PreparedStatement preparedStatement = conn.prepareStatement(query)) {
                ...
                return !resultSet.next(); // If result set is empty, room name is unique
            }
        } else {
            JOptionPane.showMessageDialog(null, "Tidak dapat terhubung ke database.", "Error", JOptionPane.ERROR_MESSAGE);
 ...
 private void combotariActionPerformed(java.awt.event.ActionEvent evt) {
        // Call the updateInstrukturCombo method when jenis tari is selected
   ...
   private boolean isTimeSlotAvailable(String kodeRuangan, String jamPinjam, String jamSelesai) {
            ...
            try (PreparedStatement preparedStatement = conn.prepareStatement(query)) {
                ...
                if (resultSet.next()) {
                    ...
                    if (!isRoomNameUnique(namaRuangan)) {
                        ...
                        return; // Exit the method if the room name is not unique
                    }
                    ...
                    try (PreparedStatement insertStatement = conn.prepareStatement(query)) {
                        ...
                        insertStatement.setString(4, jenisTariId); // Menambahkan baris ini untuk tari_id
    ...
    // Get the kodeRuangan from the selected row
