...
private void fillTableWithModifiedData() {
   ...
   private String getDanceType(String tariId) {
        ...
        try {
            ...
            String query = "SELECT a.nik, a.nama, b.tari_id " +
               ...
               "JOIN bukti_daftar b ON a.nik = b.anggota_nik";
                ...
                // Check if this nik is already present in the table
                ...
                // If the row is present, update the necessary columns
                if (isRowPresent) {
    ...
    try {
        ...
        String nik = jTable1.getModel().getValueAt(row, 0).toString(); // Get NIK directly from the table
   ...
   try {
        int row = jTable1.getSelectedRow();
        if (row >= 0) {
            String nikAnggota = jTable1.getValueAt(row, 0).toString(); // Get NIK directly from the table
